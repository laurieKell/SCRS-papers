...
```{r, base, echo=FALSE}
source("~/Desktop/SCRS-papers/mvln/R/ssMVLNFunction.R")
...
**Figure `r iFig=iFig+1; iFig`.** Kobe phase plots for MCMC, with correlations between years 
...
**Table `r iTab=iTab+1; iTab`.** Estimates of $SSB/B_{MSY}$ and  $F/F_{MSY}$ 
...
**Table `r iTab=iTab+1; iTab`.** Covariances simulated by MCMC

```{r, covar-3}
cor=cor(cast(subset(as.data.frame(mcmc1[c("stock","harvest")],drop=T),year%in%2020:2022),iter~qname+year,value="data")[,-1])
dimnames(cor)=list(vars, vars) 
